...
const headers = ref([
  ...
  { title: 'Tanggal', sortable: false, key: 'dateAdjustment' },
  { title: 'Status', sortable: false, key: 'status' },
  { title: 'Jam masuk diajukan', sortable: false, key: 'timeinAdjustment' },
  { title: 'Jam pulang diajukan', sortable: false, key: 'timeoutAdjustment' },
  { title: 'Penyetujual line', sortable: false, key: 'lineApprove' },
  ...
  <v-data-table-server v-model:items-per-page="limitlist" :headers="headers" :items="list" :items-length="totallist"
    ...
    <template v-slot:top>
      <v-toolbar color="surface">
        <v-toolbar-title>Data pengajuan koreksi absensi</v-toolbar-title>
            ...
            <v-col>
              <v-sheet class="ma-2 pa-1">
                ...
                <div class="d-flex justify-space-between">
                  ...
                  <div class="font-weight-black text-info">{{ item.hrd.name }} - {{ item.hrd.nik }}</div>
    ...
    <template v-slot:item.lineApprove="{ item }">
      <v-icon :icon="getIconData(item.lineApprove).icon" :color="getIconData(item.lineApprove).color" />
    </template>
    <template v-slot:item.hrApprove="{ item }">
      <v-icon :icon="getIconData(item.hrApprove).icon" :color="getIconData(item.hrApprove).color" />
    </template>
